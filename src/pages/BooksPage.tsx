...
import { Filter, Search, X, ChevronDown, Loader } from 'lucide-react';
...
const BooksPage: React.FC = () => {
  ...
  const categories = [
    ...
    { value: 'children', label: t('categories.children') }
  ];
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      setLoading(true);
      let query = supabase.from('books').select('*').neq('category', 'quran').neq('category', 'religious');
  ...
  const clearFilters = () => {
    const clearedFilters = { category: '', language: '', priceMin: '', priceMax: '', search: '' };
  ...
  const FilterPanel = () => (
    <div className="space-y-6">
      ...
      <div>
        ...
        <select value={filters.category} onChange={(e) => handleFilterChange('category', e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 bg-white dark:bg-leather-800 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-crimson-500">
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-leather-900">
      <div className="bg-white dark:bg-leather-800 shadow-sm border-b dark:border-gray-700">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <h1 className="text-3xl md:text-4xl font-bold text-crimson-900 dark:text-white mb-4">{t('nav.books')}</h1>
            ...
            {isFilterOpen && (
              ...
              >
                <motion.div
                  ...
                  className="absolute left-0 top-0 h-full w-72 bg-white dark:bg-leather-800 shadow-lg p-6"
